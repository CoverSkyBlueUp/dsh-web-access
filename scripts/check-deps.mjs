...
async function resolveAndReport(override) {
  ...
  switch (result.kind) {
    ...
    case 'empty': {
      // 末路兜底：尝试常见固定端口（工作区 Chrome 用 --remote-debugging-port=9222 启动的场景）
      const fallbackPort = await findFallbackPort();
      if (fallbackPort) {
        console.log(`browser: ok (port ${fallbackPort}) [通过手动调试端口连接]`);
        return { proceed: true };
      }
      console.log('browser: 未连接 — 工作区 Chrome 未启动或未开启远程调试');
      console.log('  请先用启动器打开 Chrome：node scripts/launch-chrome.mjs');
      console.log('  （DSH 沙箱内启动 Chrome 需要一次 full access 授权；启动后 Chrome 常驻，后续操作无需再授权）');
